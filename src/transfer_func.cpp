 ...
 *    Error   : -1
 **/
int transfer_func(PGconn *__con, int __idSrc, int __idDest, int __transferValue,
                  char *__sendBuf) {
  ...
  std::string sqlBegin = "BEGIN", sqlCommit = "COMMIT",
  ...
  /* 振込元口座IDチェック */
  sprintf(sql, "SELECT id, balance, min_balance FROM netbank WHERE id = %d",
          __idSrc);
  /* SQLコマンド実行 */
  ...
  /* 振込先口座IDチェック */
  sprintf(sql, "SELECT id, balance, max_balance  FROM netbank WHERE id = %d",
          __idDest);
  /* SQLコマンド実行 */
  res = PQexec(__con, sql);
  /* SQLコマンド実行結果状態を確認 */
  ...
  if (atoi(PQcmdTuples(res)) == 1) {
    sprintf(__sendBuf, "%s %d %d%s", OK_STAT, accountSrc.id, resultBalanceSrc,
            ENTER);
  }
  ...
  /* UPDATEされた行数を取得 */
  if (PQresultStatus(res) == PGRES_COMMAND_OK) {
    sprintf(__sendBuf, "%s %d %d%s", OK_STAT, accountDest.id, resultBalanceDest,
            ENTER);
  }
