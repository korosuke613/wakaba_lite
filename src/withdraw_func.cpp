 ...
 *    Error   : -1
 **/
int withdraw_func(PGconn *__con, int __id, int __withdrawValue,
                  char *__sendBuf) {
  char sql[BUFSIZE];
  ...
  /* 残高更新SQLを作成 */
  sprintf(sql, "UPDATE netbank SET balance = %d WHERE id = %d", resultBalance,
          account.id);
  /* SQLコマンド実行 */
  res = PQexec(__con, sql);
  /* SQLコマンド実行結果状態を確認 */
  ...
  if (atoi(PQcmdTuples(res)) == 1) {
    sprintf(__sendBuf, "%s %d %d%s", OK_STAT, account.id, resultBalance, ENTER);
  }
  return 0;
}
