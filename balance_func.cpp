 ...
 *    Error   : -1  
 */
int balance_func(PGconn *__con, int __id, char *__sendBuf){  
  char        sql[BUFSIZE];
  ...
  /* 残高取得SELECTのSQL文作成 */
  sprintf(sql, "SELECT id, name, balance FROM netbank WHERE id = %d", __id);
  /* SQLコマンド実行 */
  ...
  /* レスポンスメッセージ作成 */
  sprintf(__sendBuf, "%s %d %s %d%s", OK_STAT
          , account.id
          , account.name
          , account.balance
          , ENTER);
